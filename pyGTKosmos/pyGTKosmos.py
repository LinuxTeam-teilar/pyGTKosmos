...
class PyCosmos:
    ...
    def options(self, widget, window):
            ...
            if response_id   == 1:
                    ...
                    f.close()
            opt_win.destroy()
    def on_button_clicked(self, widget, sms_counter, isms_label, sms_counter_file):
            try:
                    f = open(conf_file, 'r')
                    tel = f.readline()
                    tel = tel[0:-1]
...
#                        dst = "0030" + dst_entry.get_text()
                ...
                if status == "True":
                    ...
                    try:
                        with open(sms_counter_file, 'w') as f:
                            ...
                            to_write = str(day_of_year) + ' ' + str(sms_counter)
                            f.seek(0)
                            f.truncate()
                            f.write(to_write)
    ...
    def __init__(self):
            ...
            button.connect("clicked", self.on_button_clicked, sms_counter, isms_label, sms_counter_file)
